...
export class InventoryProcessor extends WorkerHost {
  ...
  private async handleProductActivation(payload: ProductActivatedPayload) {
    this.logger.log(`[Inventory Processor Worker] Procesando Job de activación para Producto ID ${payload.productId}.`);
    ...
    if (!product) {
       this.logger.warn(`Idempotencia fallida: El producto ${payload.productId} ya no figura activo en el monolito.`);
       return;
    }
    ...
    this.logger.log(`[Inventory Processor Worker] Catálogo sincronizado exitosamente para el Producto ID ${payload.productId}.`);
