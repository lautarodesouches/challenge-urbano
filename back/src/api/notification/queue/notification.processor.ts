...
export class NotificationProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    switch (job.name) {
      ...
      default:
        this.logger.warn(
          `No handler for job ${job.name} en NotificationProcessor`,
        );
    }
  }
  ...
  private handleProductActivation(payload: ProductActivatedPayload) {
    this.logger.log(
      `Worker procesó product.activated. Producto ${payload.productId} online.`,
    );

    this.eventsGateway.broadcast('product:activated', {
  ...
  private handleProductDeactivation(payload: ProductActivatedPayload) {
    this.logger.log(
      `Worker procesó product.deactivated. Producto ${payload.productId} offline.`,
    );

    this.eventsGateway.broadcast('product:deactivated', {
      productId: payload.productId,
      isActive: payload.isActive,
      time: payload.timestamp,
    });
  ...
  private handleLowStock(payload: LowStockPayload) {
    this.logger.log(
      `Worker procesó inventory.low_stock. Alerta crítica para SKU ${payload.productId}.`,
    );
  ...
  private handlePriceChanged(payload: PriceChangedPayload) {
    this.logger.log(
      `Worker procesó product.price_changed para ${payload.productTitle}.`,
    );
